...
Phase3B / 0.6.2已授权并开始，详见[范围](docs/phase-3b.md)与[计划](docs/superpowers/plans/2026-09-10-phase-3b.md)：

- [ ] 查询父进程截止核对、迟到帧拒绝与启动失败资源清理，确定性回归。
- [ ] 宿主可关闭计时和helper逐次尝试/等待时间轴，原生分页复现与有证据的修复。
- [ ] CI保留JUnit与有界公开失败摘要，核对实际checkout和新运行结论。
- [ ] 0.6.2源码/冻结/真实数据/原生/安装验收，Git/执行说明和交接收尾。

- [ ] 为最终原生分页一次约4秒的成功读取补分层计时；区分就绪等待/宿主排队/实际查询，不从DOM响应数字推断整页速度。
