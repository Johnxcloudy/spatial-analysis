...
## Phase 3A（已授权，0.6.1 实现与验收中）

- [x] 接收用户test/真实数据、只读清点并保护原文件；范围/实施计划已保存。
- [ ] 默认无筛选分页证明与主键寻址、历史/反例及50万合成分页验收。
- [ ] 当前属性/统计页显式重试、错误不伪装空结果、新旧请求隔离。
- [ ] 真实GDB完整导入/属性几何保全/导出重读/另存重开，前后原文件哈希。
- [ ] 完整回归、冻结/原生/本机分发、Git及准确Windows CI。
- [ ] 阶段事实总结、交接/TODO与未覆盖边界收尾。
