...
try {
    ...
    Assert-SmokeCheck 'runtime.protocolVersion' ($runtime.protocolVersion -eq 7) 'The engine must report protocolVersion=7.'
    ...
    foreach ($entry in @{ saved = $saved; reopened = $reopened }.GetEnumerator()) {
        ...
        Assert-SmokeCheck "$prefix.schemaVersion" ($project.schemaVersion -eq 7) 'Project schema must be version 7.'
