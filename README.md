...
**正在验收0.6.2 / Phase3B。** 查询截止和启动回收、旧监控停止后的竞态已修复，
增加默认关闭的宿主RPC计时和CI失败摘要。当前Python326、前端113、Rust13
通过，分发和原生验收进行中，见[Phase3B范围](docs/phase-3b.md)及[执行说明](执行说明.md)。
以下0.6.1/0.6.0内容为历史交付证据，最终更新以当前验收记录为准。
...
项目入口为 `project.spa`（SQLite 元数据），托管矢量和表格使用真正的 GeoPackage，栅格使用原生 GeoTIFF，缓存与原始数据分开管理。当前应用为0.6.2，公开协议和项目schema为v6，内部worker协议为v5；打开schema1-5项目时先备份，再通过事务迁移到v6，保留历史数据集JSON和托管快照字节。不要将真实项目、缓存和诊断产物提交到代码仓库。
