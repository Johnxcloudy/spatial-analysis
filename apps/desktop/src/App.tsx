...
import { AlertCircle, ArrowRight, Check, CheckCircle2, ChevronRight, Circle, Database, File, FilePlus2, FolderOpen, Layers3, LoaderCircle, Play, RefreshCw, Save, Server, X } from 'lucide-react';
...
export default function App({ bridge = desktop }: { bridge?: DesktopBridge }) {
  ...
  const [tab, setTab] = useState<'diagnostics' | 'projection'>('diagnostics');
  ...
  return <div className="app-shell">
    <header className="app-header">
      <div className="brand"><Layers3 size={25} strokeWidth={1.6} /><div><strong>Spatial Analysis</strong><span>Desktop · Phase 0</span></div></div>
    ...
    {state.error && !state.newProject && !state.pending && <div className="error-banner" role="alert"><AlertCircle size={18} /><div><strong>{state.error.message}</strong>{state.error.data?.detail && <p>{state.error.data.detail}</p>}<span className="error-code">{state.error.data?.kind ?? 'ENGINE_ERROR'} · {state.error.code}</span></div></div>}

    <div className="workbench">
      <aside className="project-panel" aria-label="项目属性">
        <div className="panel-heading"><span><File size={16} />项目</span><span className={state.dirty ? 'save-state dirty' : 'save-state'}>{state.project ? state.dirty ? '未保存' : '已保存' : '未打开'}</span></div>
      ...
      <main className="workspace-main">
        ...
        <div className="workspace-content">
          <div className="workspace-heading"><div><span className="eyebrow">PHASE 0</span><h1>{tab === 'diagnostics' ? '本地 GIS 引擎' : '投影验证'}</h1></div><button className="button primary" disabled={!ready || locked} onClick={() => void state.diagnose()}>{state.busy === '运行 GIS 验证' ? <LoaderCircle size={16} className="spin" /> : <Play size={16} />}运行验证</button></div>
    ...
    <footer className="statusbar"><span className="status-message" role="status" aria-live="polite">{state.busy ? <><LoaderCircle className="spin" size={13} />{state.busy}</> : <><span className={`status-dot ${state.runtime ? 'online' : ''}`} />{state.notice || (state.native ? '等待引擎连接' : '本地引擎不可用')}</>}</span><span>分析 CRS · {state.draft?.analysisCrs || '未指定'}</span><span className="version-label">0.1.0</span></footer>
