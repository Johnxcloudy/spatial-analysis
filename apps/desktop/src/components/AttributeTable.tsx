...
import type { AttributePage, AttributeFilter, FeatureResult, VectorDataset } from '../../../../shared/contracts';
import type { AttributeQuery } from '../use-vector-data';
import { displayValue } from '../vector-style';

export function AttributeTable({ dataset, page, loading, query, setQuery, selectedId, selected, onSelect, onClear, enabled }: {
  dataset: VectorDataset | undefined;
  ...
  return <section className="attribute-section" aria-label="属性表">
    <div className="attribute-heading"><h2>{dataset?.name ?? '属性表'}</h2><span>{page ? `${page.total.toLocaleString('zh-CN')} 条` : dataset ? `${dataset.featureCount.toLocaleString('zh-CN')} 个要素` : '未选择图层'}</span>{loading && <LoaderCircle size={15} className="spin" />}</div>
    ...
    {selectedId !== null && <div className="selected-record"><strong>选中 ID：{selectedId}</strong>{selectedOutsidePage && <span>不在当前页</span>}<button className="icon-button" aria-label="清除要素选择" title="清除要素选择" onClick={onClear}><X size={14} /></button>{selectedOutsidePage && selected && <dl>{Object.entries(selected.row.values).map(([name, cell]) => <div key={name}><dt>{name}</dt><dd>{displayValue(cell)}</dd></div>)}</dl>}</div>}
    <div className="attribute-scroll" aria-busy={loading}>
      {page && <table><thead><tr><th className="row-id">ID</th>{fields.map((item) => <th key={item.name}><button disabled={loading || !enabled} onClick={() => changeSort(item.name)} title={`${item.name} 排序`}>{item.alias || item.name}{query.sortField === item.name && (query.descending ? <ArrowDown size={12} /> : <ArrowUp size={12} />)}</button></th>)}</tr></thead><tbody>{page.rows.map((row) => <tr key={row.id} className={selectedId === row.id ? 'selected' : ''} aria-selected={selectedId === row.id} onClick={() => onSelect(row.id)}><th><button title={`选择要素 ${row.id}`} onClick={(event) => { event.stopPropagation(); onSelect(row.id); }}>{row.id}</button></th>{fields.map((item) => <td key={item.name} title={displayValue(row.values[item.name])} className={row.values[item.name] === null ? 'null-value' : undefined}>{displayValue(row.values[item.name])}</td>)}</tr>)}</tbody></table>}
