...
export function VectorWorkspace({ bridge, state, fitRequest, onFit, visible = true }: { bridge: DesktopBridge; state: WorkspaceState; fitRequest: FitRequest | null; onFit: (bounds: [number, number, number, number]) => void; visible?: boolean }) {
  ...
  const { page, loading, error: pageError, retry: retryPage } = useAttributePage(bridge, state.project?.projectPath, vector ?? table, query, enabled, state.handleFailure);
  ...
  return <div className={`vector-workspace ${table ? 'table-workspace' : ''} ${details ? 'details-open' : ''}`}>
    <div className="map-workspace-main"><div className="map-toolbar"><span><Table2 size={15} /><strong>{table?.name ?? layer?.name ?? '地图'}</strong></span>{table ? <button className="icon-button" aria-label="生成点数据" title="生成点数据" disabled={!enabled || !!state.activeTask || !!state.busy} onClick={() => setGenerating(true)}><MapPin size={17} /></button> : <><span className="map-crs-label">EPSG:3857</span>{raster && <button className={`icon-button ${inspectPixels ? 'pressed' : ''}`} aria-label="识别像元" title="识别像元" aria-pressed={inspectPixels} disabled={!enabled || !raster.crsWkt} onClick={() => setInspectPixels(!inspectPixels)}><ScanLine size={17} /></button>}<button className="icon-button" aria-label="定位当前图层" title="定位当前图层" disabled={!dataset?.boundsWgs84 || state.copying} onClick={() => dataset?.boundsWgs84 && onFit(dataset.boundsWgs84)}><Scan size={17} /></button></>}<button className="icon-button" aria-label="导出当前数据" title={raster ? '导出 GeoTIFF' : '导出 GeoPackage'} disabled={!dataset || !enabled || !!state.activeTask || !!state.busy} onClick={() => { if (dataset) void (dataset.kind === 'raster' ? state.exportRaster(dataset.id, dataset.name) : dataset.kind === 'table' ? state.exportTable(dataset.id, dataset.name) : state.exportVector(dataset.id, dataset.name)); }}><Download size={17} /></button><button className={`icon-button ${details ? 'pressed' : ''}`} aria-label="数据与检查报告" title="数据与检查报告" aria-pressed={details} onClick={() => setDetails(!details)}><Info size={17} /></button></div>
      ...
      {raster ? <RasterPixels dataset={raster} result={pixel.result} loading={pixel.loading} enabled={enabled} onClear={() => setPixelSelection(null)} /> : <AttributeTable dataset={vector ?? table} page={page} loading={loading} error={pageError} onRetry={retryPage} query={query} setQuery={setQuery} selectedId={selectedId} selected={selected?.row.id === selectedId ? selected : null} selectedSourceRow={selection?.sourceId === sourceId ? selection?.sourceRow : undefined} enabled={enabled} onSelect={(id) => { if (sourceId) setSelection({ sourceId, featureId: id, sourceRow: page?.rows.find((row) => row.id === id)?.sourceRow }); setFitSelected(!!vector); }} onClear={() => { setSelection(null); setFitSelected(false); }} />}
    </div>
