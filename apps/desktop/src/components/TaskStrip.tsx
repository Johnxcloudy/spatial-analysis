...
const stages: Record<string, string> = { starting: '正在启动', queued: '等待执行', inspecting: '检查来源', fingerprinting: '核对来源文件', reading: '读取数据', copying: '复制数据', validating: '校验数据', writing: '写入快照', exporting: '导出数据', verifying: '验证成果', publishing: '登记成果', completed: '已完成', failed: '失败', cancelled: '已取消', interrupted: '已中断' };
...
export function TaskStrip({ tasks, disabled, onCancel }: { tasks: Task[]; disabled: boolean; onCancel: (id: string) => void }) {
  ...
  return <div className="task-strip" role="status">
    ...
    <strong>{recent.kind === 'import' ? '数据导入' : '数据导出'}</strong><span className="task-stage">{recent.error || stages[recent.status === 'running' ? recent.stage : recent.status] || recent.stage}</span>
    ...
    {sorted.length > 1 && <details className="task-history"><summary>历史 {sorted.length}</summary><ul>{sorted.map((task) => <li key={task.id}><span>{task.kind === 'import' ? '导入' : '导出'} · {stages[task.status] || task.status}</span><time>{new Date(task.createdAt).toLocaleTimeString('zh-CN')}</time>{task.error && <p>{task.error}</p>}</li>)}</ul></details>}
  </div>;
}
