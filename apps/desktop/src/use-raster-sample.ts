...
interface SampleRequest { path: string; datasetId: string; version: string; coordinate: [number, number]; signal: AbortSignal }
...
export function useRasterSample(bridge: DesktopBridge, path: string | undefined, dataset: RasterDataset | undefined, coordinate: [number, number] | null, enabled: boolean, onFailure: (cause: unknown) => void) {
  ...
  useEffect(() => {
    queue.current = latestRequest(async (request: SampleRequest) => {
      const response = await bridge.request('raster.sample', { path: request.path, datasetId: request.datasetId, coordinate: request.coordinate }, request.signal);
  ...
  useEffect(() => {
    ...
    if (enabled && path && dataset?.crsWkt && coordinate) queue.current?.push({ path, datasetId: dataset.id, version: dataset.version, coordinate, signal: controller.signal });
