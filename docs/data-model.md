...
RasterDataset 保存独立原生 GeoTIFF 快照及宽高、仿射变换、分辨率、波段类型、NoData、掩膜、单位、scale/offset、颜色解释和抽样显示范围。路径为 rasters/<UUID>.tif，版本为完整文件 SHA-256，不填造矢量字段、要素数或 ID。Raster MapLayer 另存 rasterStyle（灰度/RGB、波段、原值范围、最近邻）；矢量层保持既有 JSON 形状。显示 PNG 与原始数据分离，像元查询返回源行列和精确原值字符串。
