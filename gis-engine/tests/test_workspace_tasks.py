...
def test_table_and_points_tasks_write_private_protocol_five_requests(tmp_path: Path) -> None:
    projects, project, workspace = _stores(tmp_path)
    ...
    assert import_request["protocolVersion"] == 5
    ...
    assert points_request["protocolVersion"] == 5
...
def test_raster_tasks_write_protocol_five_requests_and_enforce_extensions(tmp_path: Path) -> None:
    projects, project, workspace = _stores(tmp_path)
    ...
    assert export_request["protocolVersion"] == 5
...
def test_worker_dispatches_raster_operations_with_exact_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    import_request.write_text(json.dumps({
        "protocolVersion": 5, "taskId": import_id, "kind": "raster_import", "payload": import_payload,
    ...
    export_request.write_text(json.dumps({
        "protocolVersion": 5, "taskId": export_id, "kind": "raster_export", "payload": export_payload,
